...
- TalkBack descriptions for calendar day cells: "{prefix}, {weekday}, {month} {day}, {count} tasks, {count} high priority, {suffix}" format. Pure `buildDayCellContentDescription` helper composes pre-localized tokens; `DayCell` resolves resources and applies `Modifier.semantics(mergeDescendants = true)`. New i18n: 2 `<plurals>` resources (tasks, high-priority) + 2 strings (today prefix, selected suffix) × 8 locales. First use of Android `<plurals>` in the project. Color is no longer the only channel for day-cell state. 10 new JVM tests in `DayCellA11yTest` covering date-only, token combinations, and defensive null paths (CAL-28, CAL-30)
