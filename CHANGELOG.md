...
- Agenda archive now offers a Snackbar with UNDO. `CalendarViewModel` exposes a one-shot `uiEvent: SharedFlow<UiEvent>`; archiving a row emits `UiEvent.ShowUndoDelete` with the archived task id. `CalendarScreen` hosts a `SnackbarHost`, collects the event, and invokes `onUndo` on action tap which routes to `ITaskManager.unarchiveTask`. For projected rows the Snackbar references the freshly-materialized row (CAL-23 materialize path). Reuses existing `action_undo` + `snackbar_task_archived` strings — no new i18n keys. 4 new VM tests (archive emits event, undo un-archives, projected-archive undo un-archives the materialized row, unknown-id is a no-op). Swipe gestures remain deferred (task list has no swipe to mirror) — archive still goes through the overflow menu (CAL-21)
