...
- Crashlytics mapping-file upload wired for release builds (FB-03) — `firebase.crashlytics { mappingFileUploadEnabled = true }` configured on the `release` build type so R8/ProGuard mapping files are uploaded to Firebase Crashlytics and the console can deobfuscate release stack traces. Closes Epic A. Authentication uses the Firebase Tools SDK's `GOOGLE_APPLICATION_CREDENTIALS` env var (Gradle can't bridge env vars to non-Exec tasks); a fail-fast pre-check on `uploadCrashlyticsMappingFile*` task start throws a clear `GradleException` when the env var is unset, instead of letting the SDK throw mid-upload. CI builds debug-only and never invokes the upload task, so credential absence is a developer-only concern. README updated with the local setup (export `GOOGLE_APPLICATION_CREDENTIALS=/path/to/key.json` before `bundleRelease`). No runtime code touched; release-build concern only (FB-03)
